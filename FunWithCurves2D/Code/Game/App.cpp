...
void App::InitialiseEngineSystems()
{
	...
	windowConfig.m_maxClientFractionOfDesktopHeight = g_gameConfigBlackboard.GetValue("clientFractionOfDesktopHeight", 0.9f);
	windowConfig.m_maxClientFractionOfDesktopWidth = g_gameConfigBlackboard.GetValue("	", 0.9f);
...
void App::RunFrame()
{
	...
	float consoleCameraOrthoY = g_gameConfigBlackboard.GetValue("uiCamOrthoSizeY", 0.0f);

	BeginFrame();
	Update();
	Render();
	
	// render the console and debug rendering content
	{
		Camera debugCamera;
		debugCamera.SetOrthoView(0.0f, 0.0f, consoleCameraOrthoX, consoleCameraOrthoY);
		DebugRenderScreenToCamera(debugCamera);
		g_theRenderer->BeginCamera(debugCamera);
		AABB2 consoleBounds = debugCamera.GetOrthoCamBoundingBox();
		g_console->Render(consoleBounds);
		g_theRenderer->EndCamera(debugCamera);
	}
