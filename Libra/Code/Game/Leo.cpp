...
Leo::Leo(Vec2 const& position, float orientationDegrees, EntityFaction faction, EntityType type, Map* map)
	: Entity(position, orientationDegrees, faction, type, map)
{
	...
	m_speed							= g_gameConfigBlackboard.GetValue("leoSpeed",						m_speed);
	m_angularVelocity				= g_gameConfigBlackboard.GetValue("leoTankAngularVelocity",			m_angularVelocity);
	m_turretLength					= g_gameConfigBlackboard.GetValue("leoTurretLength",				m_turretLength);
	m_turretCooldownTimeSeconds		= g_gameConfigBlackboard.GetValue("leoTurretCooldownTimeSeconds",	m_turretCooldownTimeSeconds);
	m_health						= g_gameConfigBlackboard.GetValue("leoHealth",						m_health);
	m_maxWanderingTime				= g_gameConfigBlackboard.GetValue("leoWanderingTime",				m_maxWanderingTime);
...
bool Leo::CheckAndFollowTargetInSight(float deltaSeconds)
{
...
void Leo::MoveTowardsTarget(float deltaSeconds)
{
...
void Leo::CheckAndFireTurretAtTargetInRange()
{
	Vec2 targetDir					= (m_targetPosition - m_position).GetNormalized();
	float targetOrientationDegrees	= targetDir.GetOrientationDegrees();
	float angularDisplaceFromTarget = GetShortestAngularDisplacementDegrees(m_orientationDegrees, targetOrientationDegrees);
	if (angularDisplaceFromTarget <= 5.0f && angularDisplaceFromTarget >= -5.0f)
	{
		if (m_turretCooldownTimer <= 0.0f)
		{
			...
			Vec2 turretWorldPosition		= m_position + g_gameConfigBlackboard.GetValue("leoTurretLocalPosition", Vec2::ZERO);
...
void Leo::Render() const
{
	...
	std::vector<Vertex_PCU> leoVertexes;
	Texture* tankTexture = g_theRenderer->CreateOrGetTexture("Data/Images/EnemyTank4.png");
	AddVertsForAABB2ToVector(leoVertexes, localSpaceBoundingBox, Rgba8::WHITE);
	TransformVertexArrayXY3D((int) leoVertexes.size(), leoVertexes.data(), 1.0f, m_orientationDegrees, m_position);
	g_theRenderer->BindTexture(0, tankTexture);
	g_theRenderer->DrawVertexArray((int) leoVertexes.size(), leoVertexes.data());
}
