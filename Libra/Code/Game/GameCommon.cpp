...
void DrawLine(Vec2 const& startPos, Vec2 const& line, Rgba8 const& color, float width)
{
...
void DrawRing(Vec2 const& centerPosition, float radius, Rgba8 const& color, float width)
{
	...
	{						
		...
		for (unsigned int index = initVertexCount; index < (initVertexCount + NUM_VERTS_QUAD); index++)
		{
			localVertexes[index].m_color = color;
		}
