...
SoundPlaybackID AudioSystem::PlaySoundAt(SoundID soundID, Vec3 const& pos, Vec3 const& velocity /*= Vec3(0.0f, 0.0f, 0.0f)*/, float volume /*= 1.f*/, bool isLooped /*= false*/)
{
	size_t numSounds = m_registeredSounds.size();
	if (soundID < 0 || soundID >= numSounds)
		return MISSING_SOUND_ID;
	...
	if (channelAssignedToSound)
	{
		FMOD_VECTOR fPos = Vec3ToFMOD(pos);
		FMOD_VECTOR fVel = Vec3ToFMOD(velocity);
		...
		channelAssignedToSound->setMode(playbackMode);
		channelAssignedToSound->setVolume(volume);
		channelAssignedToSound->setPan(0.f);
		channelAssignedToSound->setLoopCount(loopCount);
...
void AudioSystem::UpdateSoundPosition(SoundPlaybackID soundPlaybackID, Vec3 const& pos)
{
	if (soundPlaybackID == MISSING_SOUND_ID)
	{
		ERROR_RECOVERABLE("WARNING: attempt to set volume on missing sound playback ID!");
	...
	FMOD_VECTOR fPos = Vec3ToFMOD(pos);
	FMOD_VECTOR fVel = Vec3ToFMOD(Vec3(0.0f, 0.0f, 0.0f));
...
void AudioSystem::UpdateSoundPositionAndVelocity(SoundPlaybackID soundPlaybackID, Vec3 const& newPos, Vec3 const& newVel)
{
	if (soundPlaybackID == MISSING_SOUND_ID)
	{
		ERROR_RECOVERABLE("WARNING: attempt to set volume on missing sound playback ID!");
	...
	FMOD_VECTOR fPos = Vec3ToFMOD(newPos);
	FMOD_VECTOR fVel = Vec3ToFMOD(newVel);
...
void AudioSystem::UpdateListeners(int count, AudioListener const* listeners)
{
	m_fmodSystem->set3DNumListeners(count);

	for (int i = 0; i < count; i++)
	{
		FMOD_VECTOR pos = Vec3ToFMOD(listeners[i].m_position);
		FMOD_VECTOR forward = Vec3ToFMOD(listeners[i].m_forward);
		FMOD_VECTOR up = Vec3ToFMOD(listeners[i].m_up);
		FMOD_VECTOR velocity = Vec3ToFMOD(listeners[i].m_velocity);

		m_fmodSystem->set3DListenerAttributes(i, &pos, &velocity, &forward, &up);
	}
}

FMOD_VECTOR AudioSystem::Vec3ToFMOD(Vec3 vec)
{
	FMOD_VECTOR fVec;
	fVec.x = vec.x;
	fVec.y = vec.y;
